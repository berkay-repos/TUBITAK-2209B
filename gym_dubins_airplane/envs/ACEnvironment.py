...
class ACEnvironment2D:
    ...
    def takeaction(self, Nx, Nz, mu):
        ...
        self._att[0] += mu * np.deg2rad(self.roll_rate) * self.dt
        ...
        self._att[1] = np.clip(self._att[1], -10, 10)
        ...
        self._pos_m += np.array([
            self._vel_mps * cos(self._att[1]) * cos(self._att[2]),
            self._vel_mps * cos(self._att[1]) * sin(self._att[2]),
            -self._vel_mps * sin(self._att[1])
        ]) * self.dt
